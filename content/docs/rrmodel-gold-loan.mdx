...
The payload shape stays the same for every lifecycle update. The examples below are **representative stage snapshots** to show how `borrower.journey.status`, `lenders`, `offers`, and `installments` evolve over time for Gold Loan journeys.
...
```js
{
  ...
  borrower: {
    ...
    provider_id: "BORROWER-123",
    profile: {
      // same profiled borrower payload as above, including gold_loan
    },
    journey: {
      id: "jrn_1b2c3d4e-55aa-4b21-9c11-7dd2e1b8a0ab",
      type: "GOLD_LOAN",
