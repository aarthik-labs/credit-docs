...
* A **SECRET_KEY** (server-only)

<Callout type="warning" title="Never expose the SECRET_KEY in the browser">
If the SECRET_KEY appears in your frontend bundle, your integration is compromised.
...
2. Calls Aarthik’s Bootstrap Token API using the SECRET_KEY (server-to-server)
...
export async function POST(req: Request) {
  try {
    ...
    // 2) Server-to-server: call Aarthik Credit API using SECRET_KEY
    const baseUrl = process.env.AARTHIK_CREDIT_API_BASE_URL;
    const secretKey = process.env.AARTHIK_CREDIT_SECRET_KEY;
...
## 4) Frontend: open the embedded checkout
